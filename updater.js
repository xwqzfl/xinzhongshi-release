...
(function () {
  ...
  function check(manual) {
    ...
    return Promise.resolve(getChar('current')).then(function (ch) {
      ...
      if (!needScript && !needWb && !needCf && !needUpdater && !needSong) { if (manual) say('info', '已是最新版本（' + local + '）'); return; }
      var todo = [];
      ...
      var shellText = '', wbText = '', cfText = '', updText = '', songText = '';
      ...
      if (needSong) chain = chain.then(function () {
        return fetchText(RAW + 'songscript.js').then(function (t) {
          songText = t;
      ...
      return chain.then(function () {
        ...
        if (!needScript && !needCf && !needUpdater && !needSong) return wbMsg;   // 只更新世界书时不用写卡
